...
public class Game {
    ...
    /**
     * Gets the {@link Player} object which is present in the current level.
     *
    ...
    public boolean nextLevel() throws LevelException {
        ...
        if (!isFinished()) {
            if (getCurrentLevel() == 0) {
                ...
                setScore(getScore() + level.getRemainingMovements());
            }
            currentLevel++;
            loadLevel();
            return true;
    ...
    public boolean hasLostLevel() {
        return level.getRemainingMovements() == 0 && !level.hasWon();
    }
