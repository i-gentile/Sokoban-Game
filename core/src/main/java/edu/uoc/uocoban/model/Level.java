...
public class Level {
    ...
    public boolean hasWon() {
        return getMapItemList().stream().filter(mapItem -> mapItem instanceof Destination).noneMatch(d -> ((Destination) d).isEmpty());
    }
    ...
    public boolean isDeadlocked() {
        return getBoxList().stream().anyMatch(this::isDeadlocked);
    }
    ...
    private boolean isDeadlocked(Box box) {
        ...
        if (box.isDelivered()) return false;

        int x = box.getPosition().getX();
        int y = box.getPosition().getY();

        boolean isCornered = !(isPathableOrMovable(x-1, y) && isPathableOrMovable(x+1, y))
                && !(isPathableOrMovable(x, y-1) && isPathableOrMovable(x, y+1));

        boolean isBlockedByLeftBox = getMapItem(x-1,y) instanceof Box
                && !(isPathableOrMovable(x, y-1) && isPathableOrMovable(x, y+1)
                || isPathableOrMovable(x-1, y-1) && isPathableOrMovable(x-1, y+1));

        boolean isBlockedByRightBox = getMapItem(x+1,y) instanceof Box
                && !(isPathableOrMovable(x, y-1) && isPathableOrMovable(x, y+1)
                || isPathableOrMovable(x+1, y-1) && isPathableOrMovable(x+1, y+1));

        boolean isBlockedByUpBox = getMapItem(x,y-1) instanceof Box
                && !(isPathableOrMovable(x-1, y) && isPathableOrMovable(x+1, y)
                || isPathableOrMovable(x-1, y-1) && isPathableOrMovable(x+1, y-1));

        boolean isBlockedByDownBox = getMapItem(x,y+1) instanceof Box
                && !(isPathableOrMovable(x-1, y) && isPathableOrMovable(x+1, y)
                || isPathableOrMovable(x-1, y+1) && isPathableOrMovable(x+1, y+1));

        return isCornered || isBlockedByLeftBox || isBlockedByRightBox || isBlockedByUpBox || isBlockedByDownBox;
    ...
    private boolean isPathableOrMovable(int x, int y) {
        ...
        return mapItem.isPathable() || mapItem instanceof MovableEntity;
    }
